...
void sd_spi_go_high_frequency(sd_card_t *sd_card_p) {
    //printf("[DBG] enter go_high_freq\n");

    #ifndef SD_SPI_NO_HIGH_FREQ   /* ★ ここを追加 ---------------- */
        uint actual = spi_set_baudrate(sd_card_p->spi_if_p->spi->hw_inst,
                                    sd_card_p->spi_if_p->spi->baud_rate);
    ...
    /* ① dummy を 4 byte 送る → TX FIFO 完全空 ＆ まだ 80 µs 送信中 */
    uint8_t dummy[4] = {0xFF, 0xFF, 0xFF, 0xFF};
    spi_write_blocking(spi, dummy, 4);

    /* ② CS High→Low トグル */
    gpio_put(sd_card_p->spi_if_p->ss_gpio, 1);
    busy_wait_us_32(5);      // 5 µs
    gpio_put(sd_card_p->spi_if_p->ss_gpio, 0);

    /* ③ “送信完了＋BUSY安定” まで 40 µs だけ固定待ち */
    busy_wait_us_32(40);     // 400 kHz × 4 byte ＝ 80 µs 。その半分は確実に完了
    ...
    /*
    hw = spi_get_hw(sd_card_p->spi_if_p->spi->hw_inst);
    printf("[DBG] exit  BUSY=%d TFE=%d\n",
           (hw->sr & SPI_SSPSR_BSY_BITS) != 0,
           (hw->sr & SPI_SSPSR_TFE_BITS) != 0); 
    */
}
