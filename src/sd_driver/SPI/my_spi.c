...
static bool chk_spi(spi_t *spi_p) {
    spi_hw_t *hw = spi_get_hw(spi_p->hw_inst);

    /* Idle 判定を 200 µs だけ粘って待つ */
    absolute_time_t t0 = get_absolute_time();
    while (true) {
        bool busy = hw->sr & SPI_SSPSR_BSY_BITS;  /* BUSY = 1 ? */
        bool rne  = hw->sr & SPI_SSPSR_RNE_BITS;  /* RX FIFO not empty */
        bool tfe  = hw->sr & SPI_SSPSR_TFE_BITS;  /* TX FIFO empty */

        if (!busy && !rne && tfe)
            return true;              /* Idle 条件すべて OK */
        ...
        /* 200 µs 経ってもアイドルにならなければ諦めて false */
        if (absolute_time_diff_us(t0, get_absolute_time()) > 200)
            return false;
    }
}
